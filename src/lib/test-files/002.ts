import type { TestCase } from '#lib/calculateScore.test.ts'

export default (message: string): TestCase => {
	return {
		message,
		initialState: {
			cards: [
				{ played: true, rank: 'Ace', suit: 'Clubs' },
				{ played: true, rank: 'Ace', suit: 'Diamonds' },
				{ played: true, rank: '4', suit: 'Spades' },
				{ played: true, rank: '4', suit: 'Hearts', edition: 'Holographic' },
			],
		},
		expected: {
			hand: 'Two Pair',
			scoringCards: [
				{ rank: 'Ace', suit: 'Clubs' },
				{ rank: 'Ace', suit: 'Diamonds' },
				{ rank: '4', suit: 'Spades' },
				{ rank: '4', suit: 'Hearts', edition: 'Holographic' },
			],
			results: [
				{ chips: '50', multiplier: '12', score: '600', formattedScore: '600', luck: 'none' },
				{ chips: '50', multiplier: '12', score: '600', formattedScore: '600', luck: 'average' },
				{ chips: '50', multiplier: '12', score: '600', formattedScore: '600', luck: 'all' },
			],
		},
	}
}
